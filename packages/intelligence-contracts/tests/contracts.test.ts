...
describe("ADIL identifiers and locators", () => {
  ...
  it.each([
    "urn:houston:hg:prod:bamfai:media.asset",
    "urn:houston:HG:prod:bamfai:media.asset:1",
    "urn:houston:hg:prod:bamfai:media.asset:secret:value",
...
describe("ADIL contracts", () => {
  ...
  it("keeps generated JSON Schema parity for refined claims and embeddings", () => {
    ...
    const embedding = {
      ...
      id: "embedding-01",
      target: { resourceUrn: "urn:houston:hg:test:bamfai:media.asset:01JTEST" },
      model: "test-model",
      modelVersion: "1",
      dimensions: 384,
    ...
    const fact = {
      schemaVersion: "houston.dev/intelligence/v0.1",
      id: "fact-01",
      subject: { resourceUrn: "urn:houston:hg:test:bamfai:media.asset:01JTEST" },
      predicate: "media.subject",
      value: "conference",
      evidenceRefs: [{ resourceUrn: "urn:houston:hg:test:bamfai:media.asset:01JTEST" }],
      confidence: 0.9,
      kind: "fact",
  ...
  it("binds resource URNs to the manifest app, environment, and resource type", () => {
    const resource = {
      schemaVersion: "houston.dev/intelligence/v0.1",
      urn: "urn:houston:hg:test:bamfai:media.asset:01JTEST",
